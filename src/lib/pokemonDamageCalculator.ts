...
export interface Pokemon {
  ...
  ivs?: Partial<Stats>; // Individual Values (0-31)
  evs?: Partial<Stats>; // Effort Values (0-252 per stat, max 510 total)
  currentHP?: number; // Current HP for damage calculations
}
...
export const TYPE_NAME_MAPPING: Record<string, string> = {
  '一般': 'Normal',
  '格斗': 'Fighting',
  '飞行': 'Flying',
  '毒': 'Poison',
...
export class PokemonDamageCalculator {
  private typeEffectiveness: Record<string, Record<string, number>>;
   ...
   * @returns 计算后的能力值
   */
  calculateStats(pokemon: Pokemon): Stats {
    ...
    // 其他能力值计算
    const statNames: (keyof Stats)[] = ['Attack', 'Defense', 'Sp. Attack', 'Sp. Defense', 'Speed'];
   ...
   * @returns 道具修正倍率
   */
  private applyAttackerItemEffects(context: CalculationContext): number {
    ...
    // 攻击力提升道具
    const attackBoostItems: Record<string, number> = {
      '力量头带': 1.1,
      '专爱头带': 1.5,
      '生命宝珠': 1.3,
      '达人带': 1.2
    ...
    // 特定宝可梦专用道具
    const pokemonSpecificItems: Record<string, { pokemon: string; multiplier: number }> = {
   ...
   * @returns 道具修正倍率
   */
  private applyDefenderItemEffects(context: CalculationContext): number {
    ...
    // 伤害减少道具
    const damageReductionItems: Record<string, number> = {
      '突击背心': 0.67, // 特殊攻击减少1/3
      '进化奇石': 0.5
    };
    ...
    // 特定属性抗性道具
    const typeResistanceItems: Record<string, string[]> = {
      '防火服': ['火'],
      '防水服': ['水'],
      '防草服': ['草'],
      '防电服': ['电'],
   ...
   * @returns 特性修正倍率
   */
  private applyOffensiveAbilityEffects(context: CalculationContext): number {
    const { attacker, move } = context;
   ...
   */
  private applyDefensiveAbilityEffects(context: CalculationContext): { multiplier: number; typeEffectiveness: number } {
    const { defender, move } = context;
    const ability = defender.ability;
    
    if (!ability) return { multiplier: 1.0, typeEffectiveness: getTypeEffectiveness(move.type, defender.types) };
   ...
   * @returns 基础伤害值
   */
  private calculateBaseDamage(context: CalculationContext): number {
   ...
   * @returns 应用天气效果后的伤害值
   */
  private applyWeatherEffects(damage: number, context: CalculationContext): number {
   ...
   * @returns 应用墙壁效果后的伤害值
   */
  private applyScreenEffects(damage: number, context: CalculationContext): number {
    const { move, screens } = context;
    
    if (!screens || screens.length === 0) return damage;
   ...
   */
  private applyAttackerItemEffectsToStats(attacker: Pokemon, stats: Stats, moveType: string): { stats: Stats; damageMultiplier: number } {
    const item = attacker.item;
    let damageMultiplier = 1.0;
    const modifiedStats = { ...stats };
    
    if (!item) return { stats: modifiedStats, damageMultiplier };
   ...
   * @returns 修正后的能力值和伤害倍率
   */
  private applyDefenderItemEffectsToStats(defender: Pokemon, stats: Stats): { stats: Stats; damageMultiplier: number } {
    const item = defender.item;
    let damageMultiplier = 1.0;
    const modifiedStats = { ...stats };
   ...
   * @returns 修正后的各种倍率
   */
  private applyOffensiveAbilityEffectsNew(attacker: Pokemon, move: Move, otherModifiers: number, isCritical: boolean): {
    otherModifiers: number;
    stabMultiplier: number;
    criticalMultiplier: number;
    typeMultiplier: number;
    ...
    const hpTriggerMap: Record<string, string> = {
      '猛火': 'Fire',
      '激流': 'Water',
      '虫之预感': 'Bug',
      '草之毛皮': 'Grass'
   ...
   * @returns 修正后的倍率
   */
  private applyDefensiveAbilityEffectsNew(defender: Pokemon, move: Move, otherModifiers: number, typeMultiplier: number): {
    otherModifiers: number;
    typeMultiplier: number;
  } {
    const ability = defender.ability;
   ...
   * @returns 伤害计算结果
   */
  calculateDamageFromJson(jsonData: any): DamageResult {
    const attacker: Pokemon = jsonData.attacker;
    const defender: Pokemon = jsonData.defender;
    const move: Move = jsonData.move;
    const isCritical: boolean = jsonData.is_critical || false;
    const randomFactor: number = jsonData.random_factor || 1.0;
   ...
   * @returns 道具名称
   */
  getAttackerItemName(attacker: Pokemon): string {
   ...
   * @returns 道具名称
   */
  getDefenderItemName(defender: Pokemon): string {
